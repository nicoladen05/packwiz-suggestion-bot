...
export default {
    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        ...
        modal.addLabelComponents(
            ...
            buildGithubAccessTokenInputLabel(hasAccessToken),
            buildChannelInputLabel(
                isAlreadyConfigured ? currentChannel : undefined,
            ),
            buildMinVotesInput(
                isAlreadyConfigured ? currentMinVotes : undefined,
            ),
        );
...
async function isGuildAlreadyConfigured(guildId: string): Promise<{
    ...
    const serverFromDb = (
        await db
            .select({
                packwizUrl: modpack.url,
                channel: server.pollChannel,
                minVotes: server.minVotes,
                accessToken: modpack.accessToken,
            })
            .from(server)
    ...
    return {
        ...
        minVotes: serverFromDb?.minVotes,
        hasAccessToken: !!serverFromDb?.accessToken,
    };
}
...
function buildGithubAccessTokenInputLabel(isConfigured: boolean) {
    const githubAccessTokenInput = new TextInputBuilder()
        .setCustomId("githubAccessToken")
        .setStyle(TextInputStyle.Short)
        .setPlaceholder("github_pat_...")
        .setRequired(!isConfigured);
...
function buildMinVotesInput(currentValue: number | undefined) {
    const minVotesInput = new TextInputBuilder()
        .setCustomId("minVotes")
        .setStyle(TextInputStyle.Short)
        .setPlaceholder("1")
        .setRequired(true);

    if (currentValue) {
        minVotesInput.setValue(currentValue.toString());
    }

    const minVotesLabel = new LabelBuilder()
        .setLabel("Minimum votes")
        .setDescription(
            "The minimum number of votes required to approve a suggestion.",
        )
        .setTextInputComponent(minVotesInput);

    return minVotesLabel;
}
